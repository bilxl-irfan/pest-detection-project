    ...
    : N(N_), ngpus(ngpus_)
{
    ...
    Tfloat** host_bufs = new Tfloat*[ngpus];

    // Buffer size per GPU
    const size_t buf_elems = (N * N + ngpus - 1) / ngpus;
    std::cout << buf_elems << " " << ngpus << " " << N << std::endl;

    for(size_t i = 0; i < ngpus; ++i)
    {
        std::cout << "buff elem: " << buf_elems << " i = " << i << " sizeofTFloat: " << sizeof(Tfloat)  << std::endl;

        HIP_CHECK(hipSetDevice(i));
        HIP_CHECK(hipMalloc(&host_bufs[i], sizeof(Tfloat) * buf_elems));
        HIP_CHECK(hipMemset(host_bufs[i], 0, sizeof(Tfloat) * buf_elems));
        HIP_CHECK(hipDeviceSynchronize());
    }

    // Allocate device array of pointers and copy host array to it
    HIP_CHECK(hipMalloc(&bufs, sizeof(Tfloat*) * ngpus));
    HIP_CHECK(hipMemcpy(bufs, host_bufs, sizeof(Tfloat*) * ngpus, hipMemcpyHostToDevice));
    ...
    ~gpubuf_vec()
{
    // Allocate host-side copy to retrieve device pointers
    Tfloat** host_bufs = new Tfloat*[ngpus];
    HIP_CHECK(hipMemcpy(host_bufs, bufs, sizeof(Tfloat*) * ngpus, hipMemcpyDeviceToHost));

    for(size_t i = 0; i < ngpus; ++i)
    {
        HIP_CHECK(hipSetDevice(i));
        HIP_CHECK(hipFree(host_bufs[i]));
    }
